...
|-----|-----------|-------|
| **Prior to 1961** | **General Teaching Credential** | Early general-purpose teaching document. |
| **1961–1974** | **Standard Teaching Credentials** | Replaced the General Teaching Credential; standardized preparation. |
| **1970–2003** | **Ryan Credential** | Named for the Ryan Act (1970); introduced the Single Subject / Multiple Subject distinction and subject matter competence requirements. |
| **After 2003** | **SB 2042 Teaching Credentials** | Current generation; introduced the Preliminary → Clear two-tier structure and standards-based preparation. |
| — | **Supplementary / Subject Matter Authorization** | Add-on authorizations that expand a credential's subject scope. |
...
- The **authorization statement** corresponds to the authorization code and provides a detailed text explanation of what services are authorized.

These codes and statements are the basis of the `data/credential-authorizations.csv` file in this repo (1,275 rows mapping each credential code to its authorization code and text).
